...
void dump_pkg_full(PM_PKG *pkg, int level)
{
	...
	if(level > 1) {
		...
		fprintf(stdout, "\n");
		pmlist_t *backup = pacman_pkg_getinfo(pkg, PM_PKG_BACKUP);
		for(pmlist_iterator_t *i = pacman_list_begin(backup), *end = pacman_list_begin(backup); i != end; i = pacman_list_next(i)) {
			struct stat buf;
			char path[PATH_MAX];
