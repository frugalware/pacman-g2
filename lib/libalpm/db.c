...
pmdb_t *_alpm_db_open(char *dbpath, char *treename, int mode)
{
	...
	if((db->path = (char *)malloc(strlen(dbpath)+strlen(treename)+2)) == NULL) {
		RET_ERR(PM_ERR_MEMORY, -1);
	}
...
int _alpm_db_read(pmdb_t *db, char *name, unsigned int inforeq, pmpkg_t *info)
{
	...
	if(info->name[0] == 0) {
		if(_alpm_pkg_splitname(name, info->name, info->version, PM_PKG_WITHOUT_ARCH) == -1) {
			return(-1);
		}
	}
...
int _alpm_db_write(pmdb_t *db, pmpkg_t *info, unsigned int inforeq)
{
	...
	if(inforeq & INFRQ_DESC) {
		...
		if(local) {
			...
			if(info->reason && info->sha1sum[0]) {
				fprintf(fp, "%%SHA1SUM%%\n"
					"%s\n\n", info->sha1sum);
			} else if (info->reason && info->md5sum[0]) {
				fprintf(fp, "%%md5SUM%%\n"
					"%s\n\n", info->md5sum);
			}
		}
