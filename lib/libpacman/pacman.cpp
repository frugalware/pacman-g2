...
void *pacman_trans_getinfo(unsigned char parm)
{
	...
	switch(parm) {
		...
		case PM_TRANS_PACKAGES: data = !trans->packages.empty() ? c_cast(trans->packages) : c_cast(trans->syncpkgs); break;
...
pmlist_iterator_t *pacman_list_iterator_next(pmlist_iterator_t *iterator)
{
	...
	return (pmlist_iterator_t *)((FPtrListIterator *)iterator)->next();
}
...
void *pacman_list_iterator_getdata(pmlist_iterator_t *iterator)
{
	...
	return f_ptrlistitem_data((FPtrListIterator *)iterator);
}
/** @} */
